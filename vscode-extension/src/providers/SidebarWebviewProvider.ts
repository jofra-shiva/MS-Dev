...
export class SidebarWebviewProvider implements vscode.WebviewViewProvider {
  public static readonly viewId = 'msdevSidebarWebview';

  private _view?: vscode.WebviewView;
  private _projects: Project[] = [];
  private _tasksByProject = new Map<string, Task[]>();
  ...
  private _getWebAppUrl(): string {
    ...
    const cfg = vscode.workspace.getConfiguration('msdev').get<string>('webAppUrl');
    if (cfg && cfg.trim()) { return cfg.replace(/\/$/, ''); }
    // 2. Auto-detect from any project's liveUrl
    for (const p of this._projects) {
      const liveUrl = (p as any).liveUrl as string | undefined;
      if (liveUrl) {
        try {
