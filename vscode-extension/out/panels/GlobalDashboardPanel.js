...
exports.GlobalDashboardPanel = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const crypto = __importStar(require("crypto"));
const taskService_1 = require("../firebase/taskService");
class GlobalDashboardPanel {
    static createOrShow(extensionPath, user) {
        const column = vscode.window.activeTextEditor
            ? vscode.window.activeTextEditor.viewColumn
            : vscode.ViewColumn.One;
        if (GlobalDashboardPanel.currentPanel) {
            GlobalDashboardPanel.currentPanel._panel.reveal(column);
            return;
        }
        const panel = vscode.window.createWebviewPanel('msdevGlobalDashboard', 'MSDEV Dashboard', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [vscode.Uri.file(path.join(extensionPath, 'src', 'webview'))],
            retainContextWhenHidden: true,
        });
        GlobalDashboardPanel.currentPanel = new GlobalDashboardPanel(panel, extensionPath, user);
    }
    ...
    async _loadData() {
        try {
            ...
            console.error('[MSDEV] Failed to load global dashboard data:', err);
        }
    }
    _pushAll() {
        ...
        switch (msg.command) {
            ...
            case 'createProject': {
                try {
                    ...
                    const projectIds = await (0, taskService_1.getUserProjectIds)(this._user.uid);
                    this._projects = await (0, taskService_1.getProjects)(projectIds);
                    // The new project will naturally have 0 tasks, so it doesn't instantly affect subscribeToAllTasks,
                    // but we still want to push the updated project list to the dashboard immediately.
                    this._pushAll();
    ...
    _getHtmlContent() {
        const htmlPath = path.join(this._extensionPath, 'src', 'webview', 'global-dashboard.html');
    ...
    dispose() {
        this._unsubTasks?.();
        GlobalDashboardPanel.currentPanel = undefined;
        this._panel.dispose();
        this._disposables.forEach(d => d.dispose());
        this._disposables = [];
    }
}
exports.GlobalDashboardPanel = GlobalDashboardPanel;
//# sourceMappingURL=GlobalDashboardPanel.js.map