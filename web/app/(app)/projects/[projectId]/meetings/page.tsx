...
export default function MeetingsPage({ params }: { params: Promise<{ projectId: string }> | { projectId: string } }) {
  ...
  const handleSaveMeeting = async (e: React.FormEvent, meetingData: any, isNew: boolean) => {
    ...
    try {
      ...
      const existing = meetings.find(m => m.date.toISOString().split('T')[0] === dateStr);
      
      if (isNew && existing) {
        toast.error(`A meeting already exists on ${dateStr}!`);
        return;
      }
      if (!isNew && editingMeeting && existing && existing.id !== editingMeeting.id) {
        toast.error(`A meeting already exists on ${dateStr}!`);
        return;
      }
    ...
    <>
      <div className="max-w-5xl mx-auto pb-12 animate-fadeIn" style={{ paddingTop: '10px' }}>
        ...
        <div style={{ display: 'grid', gap: 20 }}>
          ...
          {[...meetings].sort((a, b) => b.date.getTime() - a.date.getTime()).map(m => {
              ...
              <div key={m.id} style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button className="btn-icon btn-ghost" onClick={() => setMeetingToDelete(m.id)} style={{ color: 'var(--danger)' }} title="Delete">
      ...
      {isCreating && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setIsCreating(false)} style={{ alignItems: 'flex-start', paddingTop: '10vh' }}>
          <div style={{ maxWidth: 600, width: '100%', margin: '0 20px', zIndex: 100 }}>
            <MeetingEditor 
              projectMembers={members}
              defaultLink={lastMeetingLink}
      ...
      {editingMeeting && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setEditingMeeting(null)} style={{ alignItems: 'flex-start', paddingTop: '10vh' }}>
          <div style={{ maxWidth: 600, width: '100%', margin: '0 20px', zIndex: 100 }}>
            <MeetingEditor 
      ...
      {showDailyMeetingPrompt && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowDailyMeetingPrompt(false)} style={{ alignItems: 'flex-start', paddingTop: '10vh' }}>
          <div style={{ maxWidth: 600, width: '100%', margin: '0 20px', zIndex: 100 }}>
            <MeetingEditor 
              isQuickLog={true}
              projectMembers={members}
      ...
      {meetingToDelete && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setMeetingToDelete(null)} style={{ alignItems: 'flex-start', paddingTop: '15vh' }}>
          <div className="modal" style={{ maxWidth: 400 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 18, fontWeight: 700 }}>Delete Meeting</h3>
              <button className="btn-icon btn-ghost" onClick={() => setMeetingToDelete(null)}>✕</button>
            </div>
            <p style={{ color: 'var(--text-2)', fontSize: 14, marginBottom: 24, lineHeight: 1.5 }}>
              Are you sure you want to delete this meeting? Tasks assigned during it will remain, but they will lose their meeting association.
            </p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button className="btn btn-primary" style={{ background: 'var(--danger)' }} onClick={() => {
                deleteMeeting(projectId, meetingToDelete);
...
function MeetingEditor({ initialData, projectMembers, onCancel, onSave, defaultLink = '', defaultName = '', isQuickLog = false }: any) {
  ...
  const handleSelectAll = () => setForm(f => ({ ...f, attendees: projectMembers.map(([uid]: any) => uid) }));
  const handleSelectNone = () => setForm(f => ({ ...f, attendees: [] }));
    ...
    <form onSubmit={handleSubmit} style={{ 
      ...
      {isQuickLog && (
        <p style={{ color: 'var(--text-2)', fontSize: 14, marginBottom: 20, lineHeight: 1.5 }}>
          No meeting has been recorded for today yet. Do you want to store a record for this session so you can track action items against it? Select who to invite with the link!
        </p>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div className="input-group">
          <label className="input-label" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            Meeting Name
            {!initialData && form.name === defaultName && defaultName && !isQuickLog && (
              <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--accent)', background: 'rgba(99,102,241,0.1)', padding: '1px 7px', borderRadius: 99 }}>Auto-suggested</span>
            )}
          </label>
        ...
        <div className="input-group" style={{ gridColumn: '1/-1' }}>
          <label className="input-label" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {!initialData && defaultLink && !isQuickLog && (
          ...
          <input className="input" name="meeting-link-input" type="url" value={form.link} onChange={e => setForm({...form, link: e.target.value})} placeholder="https://meet.google.com/..." />
        </div>
        <div className="input-group" style={{ gridColumn: '1/-1' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <label className="input-label">Attendees / Notifications</label>
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" onClick={handleSelectAll} style={{ fontSize: 11, background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontWeight: 600 }}>Select All</button>
              <button type="button" onClick={handleSelectNone} style={{ fontSize: 11, background: 'none', border: 'none', color: 'var(--text-3)', cursor: 'pointer', fontWeight: 600 }}>None</button>
            </div>
          </div>
            ...
            {projectMembers.map(([uid, m]: any) => (
              ...
              >
                {m.photoURL ? (
                  ...
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 24 }}>
        {isQuickLog ? (
          ...
          <>
            <button type="button" className="btn btn-secondary" onClick={onCancel}>Cancel</button>
            <button type="submit" className="btn btn-primary">{initialData ? 'Save Changes' : 'Save Meeting'}</button>
          </>
        )}
      </div>
    </form>
